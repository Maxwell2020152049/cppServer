...
int main() {
    Socket* sock = new Socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);

    int sock_fd = sock->get_fd();

    sock->sock_bind(inet_addr("127.0.0.1"), AF_INET, htons(4399));

    sock->sock_listen(SOMAXCONN);
    ...
    // 设置套接字为不阻塞模式
    handle_error(
        fcntl(sock_fd, F_SETFL, fcntl(sock_fd, F_GETFL) | O_NONBLOCK) == -1,
        "设置socket状态失败");

    handle_error(epoll_ctl(epfd, EPOLL_CTL_ADD, sock_fd, &ev) == -1,
        "添加sock_fd到epoll失败");
    ...
    while (true) {
        ...
        for (int i = 0; i < nfds; i++) {
            if (events[i].data.fd == sock_fd) {
                ...
                // 打印连接服务器的客户端的信息
                std::cout <<
                "client's address is " << inet_ntoa(clnt_addr.sin_addr) <<
                ", " <<
                ...
                handle_error(
                    epoll_ctl(epfd, EPOLL_CTL_ADD, clnt_fd, &ev) == -1,
                    "添加clnt_fd到epoll失败");
                ...
                while (true) {
                    ...
                    if (bytes_read > 0) {
                        std::cout <<
                            ...
                            std::endl;
                        handle_error(
                        write(events[i].data.fd, buffer, sizeof buffer) == -1,
                        "write失败");
                        ...
                        std::cout <<
                            ...
                            events[i].data.fd <<
                            ")断开连接" << std::endl;
                        handle_error(close(events[i].data.fd) == -1,
                            "关闭epoll上的套接字失败");
                        break;
                    }
                }
